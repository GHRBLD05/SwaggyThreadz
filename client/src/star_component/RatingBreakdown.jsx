...
import averageRating from './MetaAction.js';
...
export const RatingBreakdown = ({ getMetaData, dispatch }) => {
  ...
  const sumofRatings = Object.values(allRatings)
    ...
    .reduce(sumReducer);
  const averageStars =
    totalRatings > 0 ? parseFloat((sumofRatings / totalRatings).toFixed(1)) : 0;
  dispatch(averageRating(averageStars));

  // Calculate Percent Recommended
  const allRecommendations = { ...defaultRecommended, ...recommended };
  const totalRecommendations = Object.values(allRecommendations).reduce(
    sumReducer
  );
  const percentRecommended =
    totalRecommendations > 0
      ? parseFloat(
          ((allRecommendations['0'] / totalRecommendations) * 100).toFixed(1)
        )
      : 0;
...
const ConnectedRatingBreakdown = connect(
  mapStateToProps,
  null
)(RatingBreakdown);
